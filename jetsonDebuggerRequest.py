...
import requestTransmitterImplement as serialTransmitter
from Tkinter import *
...
def load():
    serialTransmitter.loadBalls()


def pause():
    serialTransmitter.stopLoadMotor()
...
class Application(Frame):
    def __init__(self, master, queue, endCommand):
        ...
        self.loadButton = Button(self, text='开始', command=load)
        self.loadButton.grid(row=7, column=0)
        self.pauseButton = Button(self, text='结束', command=pause)
        self.pauseButton.grid(row=7, column=1)
...
class ThreadedClient():
    ...
    def endApplication(self):
        self.running = False
        print('end')
        serialTransmitter.shutdown()
        serialTransmitter.destroy()


serialTransmitter.init()
serialTransmitter.start()
serialTransmitter.loadBalls()
